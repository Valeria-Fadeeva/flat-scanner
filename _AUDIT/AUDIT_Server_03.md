...
1. **C1** ✅ — Валидация путей к каталогам при старте (`main.rs:91-96`).
2. **C2** ✅ — Добавление обязательных TIFF-тегов (`ccitt_encoder.rs`).

### Средние (P1) — ВЫПОЛНЕНО
3. **M1** ✅ — Замена `ALGO_HINT_APPROX` на `ALGO_HINT_DEFAULT` для production.
4. **M2** ✅ — Оптимизация `detect_spine_shadow` через OpenCV `reduce`.
5. **M3** ✅ — Система миграций БД (`session_store.rs:235-320`).
6. **M4** ✅ — Лимит на размер загружаемого изображения 50MB (`main.rs:163-165`).

### Низкие (P2) — ВЫПОЛНЕНО
7. **L1** — Пул буферов для Sauvola (отложено).
8. **L2** — Проверка `from_str_lenient` на edge cases (отложено).
9. **L3** — Сжатие изображений в PDF (отложено).
10. **L4** ✅ — Таймауты на SANE-операции (`sane_core.rs:1-10, 75-105`).
11. **L5** ✅ — Обработка ошибок записи в `write_queue` с ретраями (`write_queue.rs:1-100`).

### Документация (P2)
12. Создать недостающие `docs/tools/*.md` для всех CV-модулей (отложено).

---

## 7. ИТОГОВЫЙ СТАТУС

**Дата завершения:** 20.08.2026  
**Сборка:** ✅ Успешна (только предупреждения об неиспользуемом коде)  
**Исправлено:** 8 из 12 проблем (все P0 и P1, L4, L5)  
**Отложено:** L1, L2, L3, документация (P2)
